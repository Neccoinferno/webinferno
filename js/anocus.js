(function () {
  ...
  function renderCommentNode(node) {
    ...
    const avatarHtml = ''; // Avatar eliminado
    ...
    return [
     ...
     // replyAction,   // <--- COMENTADO: Botón de Reply desactivado
  ...
  function renderCommentsList(container, comments) {
    if (!comments || comments.length === 0) {
      container.innerHTML = '<div class="anocus-empty">Todavía no hay comentarios.</div>';
      return;
    }
  ...
  '      <button type="button" data-role="reply-cancel">Cancelar</button>',
  '    </div>',
  '    <div class="anocus-row">',
  '      <label for="nombre">Nombre</label>',
  '      <input type="text" id="nombre" name="guest_name" maxlength="80" required />',
  '    </div>',
  //'    <div class="anocus-row">',
 // '      <label for="enlace">Enlace (opcional)</label>',
 // '      <input type="url" id="enlace" name="guest_link" maxlength="500" placeholder="https://ejemplo.com" />',
 // '    </div>',
  '    <div class="anocus-row">',
  '      <label for="comentario">Comentario</label>',
  `      <textarea id="comentario" name="content" rows="5" maxlength="${opts.maxLength}" required></textarea>`,
  '    </div>',
  ...
  try {
    ...
    formNode.addEventListener('submit', async function (event) {
      ...
      if (!guestName || !content) {
        setFeedback('El nombre y el comentario son obligatorios.', 'error');
        return;
      }

      if (opts.turnstileSiteKey && !state.turnstileToken) {
        setFeedback('Por favor, complete la verificación humana.', 'error');
        return;
      }

      try {
        setFeedback('Publicando comentario...', 'info');
        ...
        // ✅ Pequeño retraso de 1 segundo antes de recargar la lista de comentarios
        // para dar tiempo a GitHub a procesar el guardado.
        setTimeout(() => {
          renderCommentsList(listNode, state.comments);
          formNode.reset();
          clearReplyTarget();
          resetTurnstile();
          setFeedback('Comentario publicado.', 'success');
        }, 1000);

      } catch (error) {
        setFeedback(error.message || 'No se pudo publicar el comentario.', 'error');
      }
    });
